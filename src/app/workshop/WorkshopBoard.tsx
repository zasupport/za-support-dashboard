...
const STATUS_GROUPS = ['open', 'in_progress', 'waiting_parts', 'completed'];

export function WorkshopBoard({ initialJobs }: { initialJobs: Job[] }) {
  const [jobs, setJobs]   = useState<Job[]>(initialJobs);
  const [saving, setSaving] = useState<string | null>(null);

  async function cycleStatus(job: Job) {
    const next = STATUS_CYCLE[job.status] ?? 'open';
    setSaving(job.job_ref);
    try {
  ...
  function JobCard({ job }: { job: Job }) {
    return (
      <div className="border border-slate-700 rounded-md p-3 bg-slate-800/60 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs text-white font-medium leading-snug flex-1">{job.title}</p>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <span className="font-mono">{job.job_ref}</span>
          <span>·</span>
          <span>{job.client_id}</span>
          {job.serial && <><span>·</span><span className="font-mono">{job.serial}</span></>}
        </div>
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={saving === job.job_ref || job.status === 'cancelled'}
            className={`text-xs px-2 py-0.5 rounded border font-medium cursor-pointer transition-opacity hover:opacity-80 disabled:opacity-40 ${STATUS_STYLE[job.status] ?? STATUS_STYLE.open}`}
          >
            {saving === job.job_ref ? '...' : STATUS_LABEL[job.status] ?? job.status}
          </button>
        </div>
        {job.scheduled_date && (
          <p className="text-xs text-slate-500">Scheduled: {new Date(job.scheduled_date).toLocaleDateString('en-ZA')}</p>
        )}
      </div>
    );
  }
